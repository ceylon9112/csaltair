...
export default function App() {
  ...
  const errors = useMemo(
    () => (inputs ? validateInputs(inputs, thresholds) : {}),
    [inputs, thresholds],
  )

  const result = useMemo(() => {
    if (!inputs || !hasCalculated || hasErrors(errors)) return null
    return calculateRoi(inputs, thresholds, OUTCOME_TEXT)
  }, [inputs, thresholds, hasCalculated, errors])

  function selectVertical(id: VerticalId) {
    if (inputs && vertical && vertical !== id) {
      const ok = window.confirm('Changing the template will reset your inputs. Continue?')
      if (!ok) return
    }
    setVertical(id)
    setInputs(buildInputsFromVertical(id))
  ...
  function patchThresholds(patch: Partial<RoiThresholds>) {
    setThresholds((prev) => ({ ...prev, ...patch }))
  }

  function handleCalculate() {
    if (!inputs || hasErrors(errors)) return
      ...
      <div className="app">
        ...
        <section className="section vertical-picker">
          ...
          <div className="vertical-grid">
            {VERTICALS.map((id) => (
              <button key={id} type="button" className="vertical-card" onClick={() => selectVertical(id)}>
  ...
  return (
    <div className="app">
      <HeaderBanner
        ...
        subtitle={
          <p className="subtitle no-print">
            ...
            <button type="button" className="link-btn" onClick={() => setVertical(null)}>
              Change vertical
            </button>
          </p>
        }
      ...
      <section className="section collapsible no-print">
        ...
        {thresholdsOpen && (
          <div className="section-grid threshold-panel">
            ...
            <Field
              label="Strong ROI — max payback (months)"
              required
      ...
      <div className="actions no-print">
        ...
        {!canCalculate && (
          <p className="action-hint">Fix highlighted fields before calculating.</p>
        )}
      </div>
      ...
      {/* Live recalc: when hasCalculated, result memo updates automatically on input change */}
    </div>
  )
}
...
function AssumptionsEditor({
  ...
  const rows: {
    ...
    },
    {
      ...
      source: inputs.shrinkageSavingsPct === defaults.shrinkageSavingsPct ? 'vertical default' : 'user-entered',
    ...
    },
    {
      ...
      source: inputs.vandalismSavingsPct === defaults.vandalismSavingsPct ? 'vertical default' : 'user-entered',
    ...
    },
    {
      ...
      source: inputs.taskHoursReductionPct === defaults.taskHoursReductionPct ? 'vertical default' : 'user-entered',
            ...
            <td>
              {row.editable ? (
                <NumberInput
                  ...
                  suffix={row.label.includes('%') ? '%' : row.label.includes('cost') ? undefined : undefined}
